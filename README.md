<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Poppins&weight=700&size=35&pause=1000&color=00FFFF&center=true&vCenter=true&width=1000&lines=Ashwin+Selvaraj;Full-Stack+Developer;AI+Enthusiast;Building+Real-World+Solutions;Future+Software+Engineer)](https://git.io/typing-svg)
<br>

<img src="https://img.shields.io/badge/STATUS-ACTIVE-success?style=for-the-badge&colorA=1a1b27&colorB=C792EA"/>

<img src="https://img.shields.io/badge/FOCUS-FULLSTACK%20SYSTEMS-blueviolet?style=for-the-badge&colorA=1a1b27"/>

<img src="https://img.shields.io/badge/MODE-BUILDING-important?style=for-the-badge&colorA=1a1b27"/>

</div>

---

# <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30"/> SYSTEM.INFO

<div align="center">

```yaml
NAME: Ashwin Selvaraj
ROLE: Full-Stack Developer
SPECIALIZATION: Backend + Scalable Web Apps
STACK: Python | JavaScript | SQL | Supabase
MISSION: Build systems that solve practical problems
STATUS: ONLINE
```

</div>

---

# ⚡ ABOUT_ME.js

<div align="center">

<img align="right" width="350" src="https://media.giphy.com/media/ZVik7pBtu9dNS/giphy.gif"/>

</div>

```javascript
const ashwin = {
    role: "Full-Stack Developer",
    focus: [
        "Backend Systems",
        "Web Applications",
        "Database Architecture",
        "AI Integrations"
    ],

    currentlyLearning: [
        "Advanced Backend Engineering",
        "Scalable SaaS Architecture",
        "AI-powered Applications"
    ],

    freelancing: true,
    openFor: ["Web Projects", "Management Systems", "Final Year Projects"],

    mindset: "Build > Test > Optimize > Scale"
};
```

<br clear="right"/>

---

# 🚀 FEATURED PROJECTS

<div align="center">

<table>
<tr>
<td width="50%">

## 🚑 Emergency Route Optimizer

<img src="https://img.shields.io/badge/STATUS-COMPLETED-8A2BE2?style=for-the-badge"/>

<br><br>

```yaml
TYPE: Smart Navigation System
STACK: HTML + CSS + JS + Leaflet
```

### ⚡ FEATURES
```diff
+ Interactive live map rendering
+ Smart emergency path logic
+ Responsive UI/UX
+ Dynamic route visualization
+ Optimized navigation workflow
```

### 🛠 STACK

<p>
<img src="https://skillicons.dev/icons?i=html,css,javascript"/>
</p>

<img src="https://img.shields.io/badge/Leaflet.js-1B1F23?style=for-the-badge&logo=leaflet&logoColor=7CFC00"/>

<br><br>

<a href="https://github.com/AshwinSelvaraj-19/ERO">
<img src="https://img.shields.io/badge/VIEW_PROJECT-C792EA?style=for-the-badge&logo=github&logoColor=black"/>
</a>

</td>

<td width="50%">

<img src="https://media.giphy.com/media/f3iwJFOVOwuy7K6FFw/giphy.gif" width="100%"/>

</td>
</tr>
</table>

</div>

---

<div align="center">

<table>
<tr>

<td width="50%">

<img src="https://media.giphy.com/media/13HgwGsXF0aiGY/giphy.gif" width="100%"/>

</td>

<td width="50%">

## 🎓 Scholarship Finder

<img src="https://img.shields.io/badge/STATUS-ACTIVE-C792EA?style=for-the-badge"/>

<br><br>

```yaml
TYPE: AI-Assisted Platform
STACK: HTML + CSS + JS + Supabase
```

### ⚡ FEATURES
```diff
+ Smart scholarship filtering
+ Responsive dashboard
+ AI-assisted guidance flow
+ Database-powered architecture
+ User-friendly interface
```

### 🛠 STACK

<p>
<img src="https://skillicons.dev/icons?i=html,css,javascript,supabase"/>
</p>

<br>

<a href="https://github.com/AshwinSelvaraj-19/SCHOLARSHIPFINDER">
<img src="https://img.shields.io/badge/VIEW_PROJECT-C792EA?style=for-the-badge&logo=github&logoColor=black"/>
</a>

</td>

</tr>
</table>

</div>

---

# ⚔️ PROJECT ARCHIVE

<div align="center">

<table>
<tr>
<th>PROJECT</th>
<th>TECH</th>
<th>STATUS</th>
</tr>

<tr>
<td>🤖 AI Chatbot</td>
<td>OpenRouter + JavaScript</td>
<td>✅ ONLINE</td>
</tr>

<tr>
<td>🅿️ Smart Parking System</td>
<td>Supabase + JS</td>
<td>✅ ACTIVE</td>
</tr>

<tr>
<td>🌐 Business Websites</td>
<td>HTML / CSS / JS</td>
<td>✅ DEPLOYED</td>
</tr>

<tr>
<td>📊 CRUD Dashboards</td>
<td>Supabase</td>
<td>✅ ACTIVE</td>
</tr>

</table>

</div>

---

# 💻 TECH STACK

<div align="center">

<img src="https://skillicons.dev/icons?i=python,javascript,java,cpp,flask,supabase,mysql,postgres,git,github,vscode"/>

<br><br>

<img src="https://github-profile-trophy.vercel.app/?username=AshwinSelvaraj-19&theme=algolia&no-frame=true&row=1&column=7"/>

</div>

---
   

# 📊 GITHUB ANALYTICS

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=AshwinSelvaraj-19&show_icons=true&theme=midnight-purple&hide_border=true&cache_seconds=1800"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=AshwinSelvaraj-19&layout=compact&theme=midnight-purple&hide_border=true&cache_seconds=1800"/>

<br>

<img width="90%" src="https://streak-stats.demolab.com?user=AshwinSelvaraj-19&theme=midnight-purple&hide_border=true"/>

</div>
```


---

# 🏆 ACHIEVEMENTS

<div align="center">

🏅 NPTEL Certified — Data Structures using Python  
🎨 Graphic Designing Certified  
⚡ Developed multiple full-stack systems  
🔥 Freelance developer for custom web projects  

</div>

---

# 🌐 CONNECT

<div align="center">

<a href="https://www.linkedin.com/in/ashwinselvaraj-19-linkdin/">
<img src="https://img.shields.io/badge/LINKEDIN-C792EA?style=for-the-badge&logo=linkedin&logoColor=black"/>
</a>

<a href="mailto:ashwinselvaraj19@gmail.com">
<img src="https://img.shields.io/badge/GMAIL-C792EA?style=for-the-badge&logo=gmail&logoColor=black"/>
</a>

<a href="https://instagram.com/phoenixlive_19">
<img src="https://img.shields.io/badge/INSTAGRAM-C792EA?style=for-the-badge&logo=instagram&logoColor=black"/>
</a>

<a href="https://github.com/AshwinSelvaraj-19">
<img src="https://img.shields.io/badge/GITHUB-C792EA?style=for-the-badge&logo=github&logoColor=black"/>
</a>

</div>

---

# 🧬 VISITOR TERMINAL

<div align="center">

<img src="https://komarev.com/ghpvc/?username=AshwinSelvaraj-19&label=PROFILE+VIEWS&color=C792EA&style=for-the-badge"/>

<br><br>

<img src="https://capsule-render.vercel.app/api?type=waving&height=140&section=footer&color=0:141321,50:1f1c2c,100:928DAB"/>

</div>
